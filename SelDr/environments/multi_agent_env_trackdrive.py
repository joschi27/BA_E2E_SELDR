...
class MultiAgentEnvTrackDrive:
    ...
    def reset(self):
        ...
        if not self.world:
            ...
            self.reward_functions = [TrackDriveRaceRewardFunction(track_index, self.evaluation_metrics_run) for track_index in range(self.num_agents)]
    ...
    def step(self, actions: List[ActionSpace]):
        ...
        for i, (action, carla_agent, reward_function) in enumerate(
            ...
            if carla_agent.enable_debug_display:
                ...
                self.debug_instance.set_debug_info("waypoint_vector", reward_function.get_relative_vector_to_next_waypoint())
            ...
            rewards[i] = reward
            if self.evaluation_metrics_run:
                truncateds[i] = sensor_data.has_collided or reward_function.get_track_completion_percentage() == 100.0
            else:
