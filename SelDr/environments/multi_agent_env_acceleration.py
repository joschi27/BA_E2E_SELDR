...
from reward.reward_function_acceleration import AccelerationRaceRewardFunction
...
class MultiAgentEnvAcceleration:

    def __init__(self, args, num_agents, mode: Mode, enable_debug_display, enable_metric_logs=False):
    ...
    def reset(self):
        ...
        if not self.world:
            ...
            self.reward_functions = [AccelerationRaceRewardFunction() for _ in range(self.num_agents)]
    ...
    def step(self, actions: List[ActionSpace]):
        ...
        for i, (action, carla_agent, reward_function) in enumerate(
            ...
            next_waypoint = reward_function.get_relative_vector_to_next_waypoint(sensor_data.start_location, sensor_data.current_location)
            state = self.create_state_for_current_mode(sensor_data, next_waypoint)
            ...
            if self.evaluation_metrics_run:
                self.debug_instance.set_debug_info("metric_current_track_completion", reward_function.get_track_completion_percentage(sensor_data.start_location, sensor_data.current_location))
            ...
            if carla_agent.enable_debug_display:
                ...
                self.debug_instance.set_debug_info("waypoint_vector", reward_function.get_relative_vector_to_next_waypoint(sensor_data.start_location, sensor_data.current_location))
                self.debug_instance.set_debug_info("current_waypoint", 0)
    ...
    def create_vision_state(sensor_data: AgentSensorData):
        ...
        speed = math.sqrt(sensor_data.velocity.x**2 + sensor_data.velocity.y**2 + sensor_data.velocity.z**2) / 30 #normalize with max speed of about 100kmh

        return sensor_data.cam_image, [normalized_accel['x'], normalized_accel['y'], normalized_accel['z'],
                                       speed, sensor_data.wheel_steering_angles[0], sensor_data.wheel_steering_angles[1]]
